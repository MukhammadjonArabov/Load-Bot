...
@router.message(F.text.func(lambda text: bool(text and ("twitter.com" in text.lower() or "x.com" in text.lower()))))
async def twitter_url(message: Message, db_user) -> None:
    ...
    await status.edit_text(text, reply_markup=quality_keyboard("twitter", url))


@router.callback_query(F.data.startswith("dl:twitter:"))
async def twitter_download(callback: CallbackQuery, db_user) -> None:
...
async def _handle_download(callback: CallbackQuery, db_user) -> None:
    ...
    caption = truncate_title(result["title"], 900)
    try:
        if file_type == "photo":
